...
void SMSGSM::GetHora(char *hora,char *minutos, char *segundos){
        gsm.SendATCmdWaitResp("AT+CCLK?\r", 500, 100, "OK", 5);
        char *data;
        data = (char *) gsm.comm_buf;
        byte i = 0;
        while(data[i] != ','){
            i++;
        }
...
void SMSGSM::GetFecha(char *dia,char *mes, char *ano){
        gsm.SendATCmdWaitResp("AT+CCLK?\r", 500, 100, "OK", 5);
        char *data;
        data = (char *) gsm.comm_buf;
        byte i = 0;
...
void SMSGSM::SetHora(char *hora,char *minutos){
    char dia[3];
    char mes[3];
    char ano[3];
    char atcmd[40];
    GetFecha(dia,mes,ano);
    sprintf(atcmd,"AT+CCLK=\"%s/%s/%s,%s:%s:00-03\"",ano,mes,dia,hora,minutos);
    gsm.SendATCmdWaitResp(atcmd, 500, 100, "OK", 5);
}

void SMSGSM::SetFecha(char *dia,char *mes,char *ano){
    char hora[3];
    char minutos[3];
    char segundos[3];
    char atcmd[40];
    GetHora(hora,minutos,segundos);
    sprintf(atcmd,"AT+CCLK=\"%s/%s/%s,%s:%s:%s-03\"",ano,mes,dia,hora,minutos,segundos);
    gsm.SendATCmdWaitResp(atcmd, 500, 100, "OK", 5);
}

uint32 SMSGSM::GetFecha(){
        char ano[3];
        char mes[3];
        char dia[3];
...
void SMSGSM::ConfigHora(){
	Serial2.flush();
			gsm.SendATCmdWaitResp("AT+CLTS=0\r", 500, 100, "OK", 5);
	        char *data;
	        data = (char *) gsm.comm_buf;
	        ...
	        data = (char *) gsm.comm_buf;
	        SerialUSB.println(data);
	        SerialUSB.println("Fin Config Hora.");
}
