...
int GSM::begin(long baud_rate){
	...
	if (AT_RESP_OK == SendATCmdWaitResp("AT", 500, 100, "OK", 5)){
		#ifdef DEBUG_ON
			SerialUSB.println("DB:CORRECT BR");
		#endif
		turnedON=true;
	}
	if(cont==3&&norep){
		SerialUSB.println("Trying to force the baud-rate to 9600\n");
		for (int i=0;i<8;i++){
		switch (i) {
			case 0:
			  Serial2.begin(1200);
			  Serial2.print("AT+IPR=9600\r");
			  break;
		...
		SerialUSB.println("ERROR: SIM900 doesn't answer. Check power and serial pins in GSM.cpp");
		return 0;
	}
	...
	if (AT_RESP_ERR_DIF_RESP == SendATCmdWaitResp("AT", 500, 100, "OK", 5)&&!turnedON){		//check OK
		#ifdef DEBUG_ON
			SerialUSB.println("DB:DIFF RESP");
		#endif
		for (int i=0;i<8;i++){
			switch (i) {
			case 0:
			...
			if (AT_RESP_OK == SendATCmdWaitResp("AT", 500, 100, "OK", 5)){
				#ifdef DEBUG_ON
					SerialUSB.println("DB:FOUND PREV BR");
				#endif
				Serial2.print("AT+IPR=");
				Serial2.print(baud_rate);
				Serial2.print("\r"); // send <CR>
				delay(500);
				Serial2.begin(baud_rate);
				delay(100);
				if (AT_RESP_OK == SendATCmdWaitResp("AT", 500, 100, "OK", 5)){
					#ifdef DEBUG_ON
						SerialUSB.println("DB:OK BR");
					#endif
				}
				turnedON=true;
				break;
...
byte GSM::IsRxFinished(void)
{
  ...
  if (rx_state == RX_NOT_STARTED) {
	...
	/*
			...
			SerialUSB.print((unsigned long)(millis() - prev_time));
			DebugPrint("\r\nDEBUG: start_reception_tmout\r\n", 0);
			SerialUSB.print(start_reception_tmout);
...
<			SerialUSB.print((unsigned long)(millis() - prev_time));
			...
			SerialUSB.print(interchar_tmout);
...
byte GSM::IsStringReceived(char const *compare_string){
  ...
  /*
			...
			for (int i=0; i<comm_buf_len; i++){
				SerialUSB.print(byte(comm_buf[i]));
			}

			DebugPrint("\r\nDEBUG: with the string: \r\n", 0);
			SerialUSB.print(compare_string);
			DebugPrint("\r\n", 0);
		#endif
	*/
	#ifdef DEBUG_ON
		SerialUSB.print("ATT: ");
		SerialUSB.println(compare_string);
		SerialUSB.print("RIC: ");
		SerialUSB.println((char *)comm_buf);
	#endif
