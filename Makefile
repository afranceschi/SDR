...
USER_MODULES := $(SRCROOT)/SDR $(SRCROOT)/SDR/GSMSHIELD  
...
#LIBMAPLE_MODULES += $(SRCROOT)/libraries/GSM
...
UPLOAD_flash := $(SUPPORT_PATH)/scripts/reset.py && \
                sleep 2                  && \
                c:/$(DFU) -a1 -d $(BOARD_USB_VENDOR_ID):$(BOARD_USB_PRODUCT_ID) -D $(BUILD_PATH)/$(BOARD).bin -R
